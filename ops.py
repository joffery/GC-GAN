...
def batch_normal(input , scope="scope" , reuse=False, isTrain = True):
    return batch_norm(input , epsilon=1e-5, decay=0.9 , scale=True, scope=scope , reuse = reuse , updates_collections=None, is_training=isTrain)
...
def residual_in(x, output_dims, kernel, strides, name_1, name_2):

    with tf.variable_scope('residual') as scope:

        conv1 = instance_norm(conv2d(x, output_dims, k_h=kernel, k_w=kernel, d_h=strides, d_w=strides, name=name_1))
        conv2 = instance_norm(conv2d(tf.nn.relu(conv1), output_dims, k_h=kernel, k_w=kernel, d_h=strides, d_w=strides, name=name_2))
        resi = x + conv2
